...
import { clearEntry } from '../../Redux/Slices/DiarySlice';
...
const Editor = () => {
    ...
    useEffect(() => {
        // a clean up function for the state
        if (entry) {
            let clearState = setTimeout(() => {
                dispatch(clearEntry());
            }, 5000)
    ...
    <>  
     <div className = "w-full lg:mx-auto lg:w-4/5 py-4">
        ...
        <footer className="mt-4 md:mt-8"> 
            ...
            {/*alerts */}
            {!error && !localError && entry?.entry && <Alert color="success" className = "mt-4">
                <span className="font-medium">Entry Saved</span> Your entry has been saved
            </Alert>}
            {error && <Alert color="failure" className = "mt-4">
                <span className="font-medium">Save failed due to a server error. Please reload the page or try again.</span> {error}
            </Alert>}
            {localError && <Alert color="failure" onDismiss={() => {setLocalError("")}} className = "mt-4">
                <span className="font-medium">Save failed due to an internal error. Please reload the page or try again.</span> {localError}
            </Alert>}
            {/* alerts */}
        </footer>
